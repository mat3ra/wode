...
`github:mat3ra/wode#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/wode/releases/download/wip-<short-sha>/wode.tgz
```
...
```bash
npm run mat3ra:install -- wode wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/wode@<release-url>` with
